...
class TestClasses(unittest.TestCase):
    def test_fit_class(self):
        par = copy.copy(flavio.parameters.default_parameters)
        ...
        with self.assertRaises(AssertionError):
            ...
            Fit('test_fit_1', par, wc, ['m_b'],  ['m_b'])
        with self.assertRaises(AssertionError):
            # non-existent fit parameter
            Fit('test_fit_1', par, wc, ['blabla'],  [])
        with self.assertRaises(AssertionError):
            # non-existent nuisance parameter
            Fit('test_fit_1', par, wc, [],  ['blabla'])
        with self.assertRaises(AssertionError):
            # non-existent Wilson coefficient
            Fit('test_fit_1', par, wc, ['m_b'],  ['m_c'], ['C_xxx'])
        fit = Fit('test_fit_1', par, wc, ['m_b'],  ['m_c'], ['CVLL_bsbs'])
    ...
    def test_bayesian_fit_class(self):
        ...
        wc = flavio.physics.eft.WilsonCoefficients()
        fit = BayesianFit('bayesian_test_fit_1', par, wc, ['m_b','m_c'],  ['m_s'], ['CVLL_bsbs','CVRR_bsbs'])
