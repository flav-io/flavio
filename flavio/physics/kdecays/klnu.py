...
def br_plnu(wc_obj, par, P, lep):
    ...
    wc['CVL_'+qiqj+lep+'nu'] += flavio.physics.bdecays.wilsoncoefficients.get_CVLSM(par, scale, nf=3)
    ms = flavio.physics.running.running.get_ms(par, scale)
    mu = flavio.physics.running.running.get_mu(par, scale)
    return br_plnu_general(wc, par, Vij, P, qiqj, lep, ms, mu, delta=delta_Plnu(par, P, lep))
