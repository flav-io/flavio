...
class TestPll(unittest.TestCase):
    def test_np(self):
        ...
        self.assertAlmostEqual(flavio.np_prediction('BR(eta_c->mue)',wc), br,delta=0.001*br)
        ...
        self.assertAlmostEqual(flavio.np_prediction('BR(eta_c/psi->taue)',wc), br,delta=0.001*br)
        
        wc,br=Wilson({'CSRR_tauecc' : 1e-2},scale=2.,eft='WET',basis='flavio'),3.5551e-5
        self.assertAlmostEqual(flavio.np_prediction('BR(eta_c->taue)',wc), br,delta=0.001*br)
