...
def getfft(s, par, B, ff, ff0, lep, wc):
    ...
    flavio.default_citations.register("Kozachuk:2017mdk")
...
def getF1(s, par, B, ff, ff0, lep, wc):
    flavio.default_citations.register("Melikhov:2004mk")
    scale = config['renormalization scale']['bllgamma']
    mb = running.get_mb(par, scale, nf_out=5)
    mbh = mb/par['m_'+B]
    bq = meson_quark[B]
...
def B10(s, par, B, ff, ff0, lep, wc):
    flavio.default_citations.register("Melikhov:2004mk")
    flavio.default_citations.register("Guadagnoli:2016erb")
