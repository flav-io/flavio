...
def wem_22_HL_low(sh, mb, ml, scale):
...
def wem_22_HT_low(sh, mb, ml, scale):
...
def wem_27_HL_low(sh, mb, ml, scale):
    return wem_27_HL_low_re(sh, mb, ml, scale) + 1j*wem_27_HL_low_im(sh, mb, ml, scale)

def wem_27_HL_low_im(sh, mb, ml, scale):
...
def wem_27_HL_low_re(sh, mb, ml, scale):
...
def wem_27_HT_low(sh, mb, ml, scale):
    return wem_27_HT_low_re(sh, mb, ml, scale) + 1j*wem_27_HT_low_im(sh, mb, ml, scale)


def wem_27_HT_low_im(sh, mb, ml, scale):
...
def wem_27_HT_low_re(sh, mb, ml, scale):
...
def wem_29_HL_low(sh, mb, ml, scale):
    return wem_29_HL_low_re(sh, mb, ml, scale) + 1j*wem_29_HL_low_im(sh, mb, ml, scale)


def wem_29_HL_low_im(sh, mb, ml, scale):
...
def wem_29_HL_low_re(sh, mb, ml, scale):
...
def wem_29_HT_low(sh, mb, ml, scale):
    return wem_29_HT_low_re(sh, mb, ml, scale) + 1j*wem_29_HT_low_im(sh, mb, ml, scale)


def wem_29_HT_low_im(sh, mb, ml, scale):
...
def wem_29_HT_low_re(sh, mb, ml, scale):
...
def wem_77_HL_low(sh, mb, ml, scale):
...
def wem_77_HT_low(sh, mb, ml, scale):
...
def wem_79_HL_low(sh, mb, ml, scale):
...
def wem_79_HT_low(sh, mb, ml, scale):
...
def wem_99_HL_low(sh, mb, ml, scale):
...
def wem_99_HT_low(sh, mb, ml, scale):
...
def wem_22_high(sh, mb, ml, scale):
...
def wem_27_high(sh, mb, ml, scale):
    return wem_27_re_high(sh, mb, ml, scale) + 1j*wem_27_im_high(sh, mb, ml, scale)


def wem_27_re_high(sh, mb, ml, scale):
...
def wem_27_im_high(sh, mb, ml, scale):
    return (((-612530674 / 1182081 + (1248725501 * (1 - sh)) / 609972 -
              (12404910453 * (1 - sh)**2) / 2775125 + (3944570424 * (1 - sh)**3) / 817063) *
             log(mb**2 / ml**2)) / 96)


def wem_27_low(sh, mb, ml, scale):
    return wem_27_re_low(sh, mb, ml, scale) + 1j*wem_27_im_low(sh, mb, ml, scale)

def wem_27_im_low(sh, mb, ml, scale):
...
def wem_27_re_low(sh, mb, ml, scale):
...
def wem_29_high(sh, mb, ml, scale):
    return wem_29_re_high(sh, mb, ml, scale) + 1j*wem_29_re_high(sh, mb, ml, scale)


def wem_29_re_high(sh, mb, ml, scale):
...
def wem_29_low(sh, mb, ml, scale):
    return wem_29_re_low(sh, mb, ml, scale) + 1j*wem_29_re_low(sh, mb, ml, scale)

def wem_29_re_low(sh, mb, ml, scale):
...
def wem_77_low(sh, mb, ml, scale):
    return (log(mb**2/ml**2)*(-sh/(2.*(-1 + sh)*(2 + sh)) + log(1 - sh) - (sh*(-3 + 2*sh**2)*log(sh))/(2.*(-1 + sh)**2*(2 + sh))))

def wem_79_low(sh, mb, ml, scale):
    return (log(mb**2/ml**2)*(1/(2.*(-1 + sh)) + log(1 - sh) - ((1 - 2*sh + 2*sh**2)*log(sh))/(2.*(-1 + sh)**2)))
...
def wem(I, i, j, sh, mb, ml, scale):
    if sh < 0.5:
        return wem_dict_low[I, i, j](sh, mb, ml, scale).real
    else:
        return wem_dict_high[I, i, j](sh, mb, ml, scale).real
