...
def _helicity_amps_deltaC(q2, deltaC, C_name, par, B, P, lep):
    ml = par['m_'+lep]
    mB = par['m_'+B]
    mP = par['m_'+P]
    ...
    wc[C_name] = deltaC
    return flavio.physics.bdecays.angular.helicity_amps_p(q2, mB, mP, mb, 0, ml, ml, ff, wc, N)

# parametrization of subleading hadronic corrections
def helicity_amps_deltaC7(q2, deltaC7, par, B, P, lep):
    ...
    """
    return _helicity_amps_deltaC(q2, deltaC7, '7', par, B, P, lep)

def helicity_amps_deltaC9(q2, deltaC9, par, B, P, lep):
    r"""A function returning a contribution to the helicity amplitudes in
    $B\to P\ell^+\ell^-$ coming from an effective shift of
...
def helicity_amps_deltaC7_polynomial(q2, par, B, P, lep):
    deltaC7   =( par[B+'->'+P+' deltaC7 a Re']  + par[B+'->'+P+' deltaC7 b Re'] *q2
             +1j*( par[B+'->'+P+' deltaC7 a Im']  + par[B+'->'+P+' deltaC7 b Im'] *q2 ))
    return helicity_amps_deltaC7(q2, deltaC7, par, B, P, lep)
...
def helicity_amps_deltaC9_polynomial(q2, par, B, P, lep):
    deltaC9   =( par[B+'->'+P+' deltaC9 a Re']  + par[B+'->'+P+' deltaC9 b Re'] *q2
             +1j*( par[B+'->'+P+' deltaC9 a Im']  + par[B+'->'+P+' deltaC9 b Im'] *q2 ))
    return helicity_amps_deltaC9(q2, deltaC9, par, B, P, lep)

# a constant shift, e.g. for high q^2
def helicity_amps_deltaC9_constant(q2, par, B, P, lep):
    deltaC9   = par[B+'->'+P+' deltaC9 c Re'] + 1j*par[B+'->'+P+' deltaC9 c Im']
    return helicity_amps_deltaC9(q2, deltaC9, par, B, P, lep)


# Functions returning functions needed for Implementation
def fct_deltaC7_polynomial(B, P, lep):
    def fct(wc_obj, par_dict, q2, cp_conjugate):
        par = par_dict.copy()
        if cp_conjugate:
            par = conjugate_par(par)
        return helicity_amps_deltaC7_polynomial(q2, par, B, P, lep)
    return fct

def fct_deltaC9_polynomial(B, P, lep):
    def fct(wc_obj, par_dict, q2, cp_conjugate):
        par = par_dict.copy()
        if cp_conjugate:
            par = conjugate_par(par)
        return helicity_amps_deltaC9_polynomial(q2, par_dict, B, P, lep)
    return fct

def fct_deltaC9_constant(B, P, lep):
    def fct(wc_obj, par_dict, q2, cp_conjugate):
        par = par_dict.copy()
        if cp_conjugate:
            par = conjugate_par(par)
        return helicity_amps_deltaC9_constant(q2, par_dict, B, P, lep)
    return fct
...
for had in [('B0','K0'), ('B+','K+'), ]:
    for l in ['e', 'mu', 'tau']:
        process = had[0] + '->' + had[1] + l+l # e.g. B0->K0mumu
        ...
        i = Implementation(name=iname, quantity=quantity,
                       function=fct_deltaC9_constant(B=had[0], P=had[1], lep=l))
