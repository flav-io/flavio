...
def alpha_ll(alpha_l1, alpha_l2):
    return sqrt(alpha_l1**2 + alpha_l2**2)
...
def formfactors(par, q2):
    # experimental masses because of small difference to model masses, differences will be covered by uncertainties
    mL = par['m_Lambda']
    mLb = par['m_Lambdab']
    m_q = par['Lambdab->Lambda(1520) m_q']
    m_s = par['Lambdab->Lambda(1520) m_s']
    alpha_l1 = par['Lambdab->Lambda(1520) alpha_Lambdab']
    alpha_l2 = par['Lambdab->Lambda(1520) alpha_Lambda(1520)']
    ...
    FormDict = {}
    for e in FList:
        a0 = par['Lambdab->Lambda(1520) '+e+' a0']
        a2 = par['Lambdab->Lambda(1520) '+e+' a2']
        a4 = par['Lambdab->Lambda(1520) '+e+' a4']
...
def ff_equivalence(par, q2):
    # transform FormDict in form factors used in arXiv:1903.00448
    FD, mL, mLb = formfactors(par, q2)
    ff = {}
    ff['fVt'] = ( FD['F2']*(mL**2 - mLb**2 - q2) + mLb*(2*FD['F1']*mL*(mL - mLb) - 2*FD['F4']*mL*mLb + FD['F3']*(mL**2 - mLb**2 + q2)))/(2*mL*(mL-mLb)*mLb**2 ) 
    ...
    ff['fT5t'] = 0
    ff['fT5perp'] = -1/(2*mL*(mL-mLb)*mLb**2*(mL**2 + 2*mL*mLb + mLb**2 - q2))*(FD["H2"]*mL*(mL**4 + (mLb**2 - q2)**2 - 2*mL**2*(mLb**2 + q2)) + mLb*(mL*(2*FD["H5"]*mLb*(mL*mLb + mLb**2 - q2) + FD["H3"]*(mL**4 + (mLb**2 - q2)**2 - 2*mL**2*(mLb**2 + q2)))) ) 
    ff['fT50'] = FD["H1"]/mLb + 2*FD["H5"]*mL/(mL**2 + 2*mL*mLb + mLb**2 - q2) 
    ff['fT5g'] = -FD["H5"]*(mL + mLb) - FD["H6"]*(mL**2 + 2*mL*mLb + mLb**2 - q2)/(2*mLb)
    
# !!! Assign uncertainties to ff's
