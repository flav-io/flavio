...
def BR_mu3e(wc_obj, par):
    ...
    pre_br = 1 / (8 * par['GF']**2)
    pre_wc_1 = 4 * par['GF'] / sqrt(2) / (16 * pi**2)
    pre_wc_2 = 4 * par['GF'] / sqrt(2) * e**2 / (16 * pi**2)
    DLg = pre_wc_1 * wc['C7_mue']
    DRg = pre_wc_1 * wc['C7p_mue']
    FLL = pre_wc_2 * wc['CVLL_mueee']
    FLR = pre_wc_2 * wc['CVLR_mueee']
    FRL = pre_wc_2 * wc['CVRL_mueee']
    FRR = pre_wc_2 * wc['CVRR_mueee']
    SRR = pre_wc_2 * wc['CSRR_mueee']
    SLL = pre_wc_2 * wc['CSLL_mueee']
    mmu = par['m_mu']
    me = par['m_e']
